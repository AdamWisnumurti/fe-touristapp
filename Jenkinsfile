// ::DEFINE  
def image_name          = "tds-stok-rumah-fe"
def service_name        = "tds-stok-rumah-fe"
def repo_name           = "tds/tds-stok-rumah-fe" 
def appName             = "tds-stok-rumah-fe"
...
// ::CLUSTER
def cluster_dev         = "https://10.3.33.235:6443"      
def cluster_staging     = "https://10.2.33.72:6443"   
def cluster_prod        = "https://10.1.32.139:6443"
def credentials_dev     = "clusterdev-alibaba"
def credentials_staging = "clusterstaging-alibaba"
...
def docker_dev          = "10.172.24.50:8082/docker-dev/tapera-digital-service"
def docker_staging      = "10.172.24.50:8083/docker-staging/tapera-digital-service"
def docker_prod         = "10.172.24.50:8084/docker-stable/tapera-digital-service"
def docker_credentials  = "ci-cd"
def helm_git            = "https://gitlab.tapera.go.id/dt/helm-tapera-digital-service.git"
def artifact_dev        = "https://nexus.tapera.go.id/repository/helm-test-tapera-digital-service"
def artifact_staging    = "https://nexus.tapera.go.id/repository/helm-incubator-tapera-digital-service"
def artifact_prod       = "https://nexus.tapera.go.id/repository/helm-stable-tapera-digital-service"
...
//def k6_repo             = "https://gitlab.tapera.go.id/dt/k6-tapera-digital-service.git"
...
def service_staging_url = "https://stagingsitaracore.tapera.go.id"

// ENDPOINTS
def endpoint_urls        = ["\"${service_staging_url}/api/kyc_ifua/File/List\"",
                            "\"${service_staging_url}/api/kyc_ifua/File/Generate\"",
                            "\"${service_staging_url}/api/kyc_ifua/File/Upload\"",
                            "\"${service_staging_url}/api/kyc_ifua/Candidate/List\""]

def config = ['{\"name\":\"CICD\",\"type\":\"NamedConfiguration\"}']
...
node ('master') {
    try {
        ...
        if (version != "release") {
             stage ("Unit Test") {
                ...
                //def unitTestGetValue = sh(returnStdout: true, script: '''
                //sed -i '/.pb.go/d' coverage.out && go tool cover -func coverage.out | grep total | awk '{print$3}'
                //''')
    ...
    stage ("SonarQube Analysis") {
        ...
        //                     -Dsonar.exclusions="**/main.go,**/*.pb.go,**/logger.go" \
        //                     -Dsonar.language=go \
...
def helmDryRun(Map args) {
    ...
    sh "helm install --dry-run --debug ${args.name} ${args.service_name} -n tds"
}

def helmDryRunGrpc(Map args) {
    echo "Running dry-run deployment"
    sh "helm install --dry-run --debug ${args.name} ${args.service_name_grpc} -n tds"
}
...
def helmUpgrade(Map args) {
    ...
    sh "helm upgrade --install ${args.name} ${args.service_name} -n tds"
    sh "kubectl rollout restart deployment ${args.service_name} -n tds"
}

def helmUpgradeGrpc(Map args) {
    echo "Upgrade chart deployment"
    sh "helm upgrade ${args.name} ${args.service_name_grpc} -n tds --recreate-pods"
}

def helmInstall(Map args) {
    echo "Install chart deployment"
    sh "helm install ${args.name} ${args.service_name} -n tds"
}

def helmInstallGrpc(Map args) {
    echo "Install chart deployment"
    sh "helm install ${args.name} ${args.service_name_grpc} -n tds"
}

def helmRollBack(Map args) {
    echo "Roolback chart deployment to Previous Version"
    sh "helm rollback ${args.name} 0 ${args.service_name} -n tds"
}

def helmRollBackGrpc(Map args) {
    echo "Roolback chart deployment to Previous Version"
    sh "helm rollback ${args.name} 0 ${args.service_name_grpc} -n tds"
}
...
def burpsuiteScan(Map args) {
    echo "Running Burpsuite Scanner"
        sh "curl -vgw -X POST ${args.burpsuite_url} -d '{\"name\": \"${args.service_name}\",\"scan_configurations\":${args.config},\"urls\": ${args.endpoint_urls}}'"
}
